    ...
    try:
        while dataset.isOk():
            ...
            if img is not None:
                ...
                R = vo.cur_R
                translation = vo.cur_t
                all_poses.append(np.concatenate([np.concatenate([vo.cur_R, vo.cur_t],1), np.array([0., 0., 0., 1.0])[None]], 0))
        ...
        print("saving")
        try:
            os.mkdir(f'{config.dataset_settings["base_path"]}/generated')
            os.mkdir(f'{config.dataset_settings["base_path"]}/generated/images')
        except OSError as error:
            print(error) 
        transforms = []
        for i in range(len(all_images)):
            cv2.imwrite(f'{config.dataset_settings["base_path"]}/generated/images/frame_{i}.png', all_images[i])
            info_json = {
                "cx": config.cam_settings['Camera.cx'],
                "cy": config.cam_settings['Camera.cy'],
                "w": config.cam_settings['Camera.width'],
                "h": config.cam_settings['Camera.height'],
                "fl_y": config.cam_settings['Camera.fy'],
                "fl_x": config.cam_settings['Camera.fx'],
                "transform_matrix": all_poses[i].tolist(),
            }
            transforms.append(info_json)
        with open(f'{config.dataset_settings["base_path"]}/generated/transforms.json', "w") as json_file:
            json.dump({"frames": transforms}, json_file)
        ...
        # with open(f'{config.dataset_settings["base_path"]}/generated/drone_video_0.pickle', 'wb') as handle:
        #     pickle.dump({"poses": all_poses, "images": all_images}, handle, protocol=pickle.HIGHEST_PROTOCOL)
